<template>
  <div class="module-content mt-24">
    <div class="expand-module w-100">
      ...
      <h6 class="d-inline-block mb-0">{{moduleData.title}}</h6>
      <a class="float-right" href="#" @click="showModule = !showModule"><img src="../../assets/img/utils/ico-arrow-dropdown.svg"></a>
      <span @click="$emit('edit-module', moduleData.id)" class="cursor-pointer pencil-ico float-right mr-4"></span>
    </div>
    <div class="expanded-module" v-show="showModule">
      <div style="margin-top: 29px; gap: 16px" class="d-flex justify-content-between">
        ...
        <button class="btn btn-block w-100 btn-rounded-purple mt-0" @click="$emit('add-lesson', moduleData.id)">+ Criar aula</button>
      </div>
      <div class="lessons mt-24">
        <draggable v-model="moduleData.classes" draggable=".item">
          <div class="lesson mt-2 item" v-for="(lesson, index) in moduleData.classes" :key="index">
            <span class="three-points mr-2"></span>
            {{ lesson.title }}
            <b-dropdown class="menu-card float-right" right toggle-class="btn-menu btn-white squad">
              ...
              <b-dropdown-item @click="$emit('open-class', {classID: lesson.id, moduleID: moduleData.id})"><i class="pencil-ico mr-1"></i>Editar aula</b-dropdown-item>
